...
export function removeItem<T extends Item>(
...
export function unselectItem<T extends Item>(
	selected: Array<T>,
	key: T
): Array<T> {
 ...
 *  changes.
 */
export function raiseSelectOnChange<T extends Item>(
	store: Selectable<T>
): Behavior {
 ...
 */
export function selectActive<T extends Item>(
	state: List<T> & Selectable<T>
): Array<T> {
 ...
 */
export function previousActiveSelectable<T extends Item>(
	state: List<T> & Selectable<T>
) {
	...
	if (state.active === null && lastSelected !== undefined) {
		const index = state.items.findIndex((x) =>
			itemsEqual(x.value, lastSelected)
		);
 ...
 *  are no selected items.
 */
export function nextActiveSelectable<T extends Item>(
	state: List<T> & Selectable<T>
) {
	const firstSelected = state.selected[0];
	if (state.active === null && firstSelected !== undefined) {
		const index = state.items.findIndex((x) =>
			itemsEqual(x.value, firstSelected)
		);
...
export function isKeySelected<T extends Item>(
	selected: Array<T>,
	key: ItemKey<T>
): boolean {
...
export function isKeyActive<T extends Item>(
	list: List<T>,
	key: ItemKey<T>
): boolean {
